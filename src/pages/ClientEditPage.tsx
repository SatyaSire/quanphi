...
import { getAllClients, updateClient } from '../data/clientsData';
...
const ClientEditPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadClientData = async () => {
      ...
      try {
        ...
        // Get client data from our data store
        ...
        setFormData({
          ...
          tags: client.tags || [],
          internalComments: client.internalComments || '',
        ...
        setErrors({ load: 'Failed to load client data. Please try again.' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update client using the data store
      const updates = {
        name: formData.name,
      ...
      const updatedClient = updateClient(id!, updates);
              ...
              <div className="flex space-x-3">
                ...
                {activeTab !== 'additional' ? (
                  <button
                    ...
                    className="px-6 py-3 bg-orange-600 text-white font-medium rounded-xl hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500/50 transition-all duration-200"
                  >
                    Next
                  </button>
                ) : (
